...
import { z } from "zod";
import { NetworkIdentifier, ChainError } from "./chains.js";
...
const networkSchema = z.string().refine(
...
const ensNameSchema = z.string().refine(
  (val) => val.includes('.'),
  "ENS name must contain a dot (e.g., 'name.eth')"
);

// Add error handling
function handlePromptError(error: unknown): PromptError {
  if (error instanceof ChainError) {
    return {
      error: error.message,
      code: error.code,
      details: { chainIdentifier: error.chainIdentifier }
    };
  }
...
// Add type definitions for prompt parameters
interface BlockExplorerParams {
  blockNumber?: string;
  network?: string;
}
...
interface AddressAnalysisParams {
  address: string;
  network?: string;
}
 ...
 */
export function registerEVMPrompts(server: McpServer): void {
  try {
  ...
  server.prompt(
    ...
    z.object({
      blockNumber: z.string().optional().describe("Block number to explore. If not provided, latest block will be used."),
        network: networkSchema.optional().describe("Network name (e.g., 'ethereum', 'optimism', 'arbitrum', 'base', etc.) or chain ID. Supports all EVM-compatible networks. Defaults to Ethereum mainnet.")
    }),
      ({ blockNumber, network = "ethereum" }: BlockExplorerParams): PromptResponse => ({
      messages: [{
        role: "user",
        content: {
          type: "text",
  ...
  server.prompt(
    ...
    z.object({
        txHash: z.string().min(1).describe("Transaction hash to analyze"),
        network: networkSchema.optional().describe("Network name (e.g., 'ethereum', 'optimism', 'arbitrum', 'base', etc.) or chain ID. Supports all EVM-compatible networks. Defaults to Ethereum mainnet.")
    }),
      ({ txHash, network = "ethereum" }: TransactionAnalysisParams): PromptResponse => ({
      messages: [{
        role: "user",
        content: {
          type: "text",
  ...
  server.prompt(
    ...
    z.object({
        address: addressSchema.describe("Ethereum address or ENS name to analyze"),
        network: networkSchema.optional().describe("Network name (e.g., 'ethereum', 'optimism', 'arbitrum', 'base', etc.) or chain ID. Supports all EVM-compatible networks. Defaults to Ethereum mainnet.")
    }),
      ({ address, network = "ethereum" }: AddressAnalysisParams): PromptResponse => ({
      messages: [{
        role: "user",
        content: {
          type: "text",
  ...
  server.prompt(
    ...
    const promptError = handlePromptError(error);
    console.error('Failed to register EVM prompts:', promptError);
    throw new Error(`Failed to register EVM prompts: ${promptError.error}`);
  }
}